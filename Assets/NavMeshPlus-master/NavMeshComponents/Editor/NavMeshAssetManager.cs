...
namespace NavMeshPlus.Components.Editors
{
    public class NavMeshAssetManager : ScriptableSingleton<NavMeshAssetManager>
    {
        ...
        {
            public NavigationSurface surface;
        ...
        static string GetAndEnsureTargetPath(NavigationSurface surface)
        {
        ...
        static void CreateNavMeshAsset(NavigationSurface surface)
        {
        ...
        NavMeshData GetNavMeshAssetToDelete(NavigationSurface navSurface)
        {
            if (PrefabUtility.IsPartOfPrefabInstance(navSurface) && !PrefabUtility.IsPartOfModelPrefab(navSurface))
            {
                // Don't allow deleting the asset belonging to the prefab parent
                var parentSurface = PrefabUtility.GetCorrespondingObjectFromSource(navSurface) as NavigationSurface;
        ...
        void ClearSurface(NavigationSurface navSurface)
        {
        ...
        public void StartBakingSurfaces(UnityEngine.Object[] surfaces)
        {
            ...
            foreach (NavigationSurface surf in surfaces)
            {
        ...
        static NavMeshData InitializeBakeData(NavigationSurface surface)
        {
        ...
        public bool IsSurfaceBaking(NavigationSurface surface)
        {
            if (surface == null)
                return false;
        ...
        {
            foreach (NavigationSurface s in surfaces)
                ClearSurface(s);
        }

        static void SetNavMeshData(NavigationSurface navSurface, NavMeshData navMeshData)
        {
        ...
        void StoreNavMeshDataIfInPrefab(NavigationSurface surfaceToStore)
        {
            ...
            {
                var basePrefabSurface = PrefabUtility.GetCorrespondingObjectFromSource(surfaceToStore) as NavigationSurface;
        ...
        bool IsCurrentPrefabNavMeshDataStored(NavigationSurface surface)
        {
            if (surface == null)
                return false;
        ...
        void DeleteStoredNavMeshDataAssetsForOwnedSurfaces(GameObject gameObjectInPrefab)
        {
            ...
            var surfaces = gameObjectInPrefab.GetComponentsInChildren<NavigationSurface>(true);
            foreach (var surface in surfaces)
                DeleteStoredPrefabNavMeshDataAsset(surface);
        }

        void DeleteStoredPrefabNavMeshDataAsset(NavigationSurface surface)
        {
        ...
        void ForgetUnsavedNavMeshDataChanges(PrefabStage prefabStage)
        {
            ...
            var allSurfacesInPrefab = prefabStage.prefabContentsRoot.GetComponentsInChildren<NavigationSurface>(true);
            NavigationSurface surfaceInPrefab = null;
            var index = 0;
            do
            {
                ...
                for (var i = m_PrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
                {
                    ...
                    else if (surfaceInPrefab != null && storedPrefabInfo.surface == surfaceInPrefab)
                    {
                        ...
                        var baseSurface = PrefabUtility.GetCorrespondingObjectFromSource(surfaceInPrefab) as NavigationSurface;
